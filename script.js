function mortgageCalculation() {
  ...
  if (isInputEmpty(APR, term, amount)) {
    ...
    return (document.getElementById("payment").value =
...
function isInputEmpty(APR, term, amount) {
  if (!APR || APR * 12 > 25) {
    document.getElementById("error-Message").innerHTML =
      "Please fill out a APR lower than 25.00 before submitting";
    return document.getElementById("APR").focus();
  }
  if (!term || term / 12 > 40) {
    document.getElementById("error-Message").innerHTML =
      "Please fill out loan term in years lower than 40 before submitting";
